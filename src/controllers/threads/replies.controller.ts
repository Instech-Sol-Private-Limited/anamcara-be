...
const createReply = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        // Fetch parent comment (get thread_id, post_id, or story_id)
        const { data: commentData, error: commentError } = await supabase
            .from('threadcomments')
            .select('id, user_id, thread_id, post_id, story_id, content')
            .eq('id', comment_id)
            .single();
        ...
        // Determine if this is a thread, post, or story comment
        ...
        if (commentData.thread_id) {
            ...
            const { data: storyData } = await supabase
                ...
                .eq('id', parentId)
                .single();
            parentTitle = storyData?.title || 'a story';
            parentAuthorId = storyData?.user_id;
        } else {
        ...
        if (commentData.user_id !== user_id) {
            ...
            if (userData?.email) {
                await sendNotification({
                    ...
                    actorUserId: user_id,
                    threadId: parentId, // use parentId for NotificationInput
                    message: `${first_name}${last_name ? ` ${last_name}` : ''} replied to your comment: **${getCommentPreview(commentData.content)}**`,
                    type: parentType === 'thread' ? 'reply' : 
                          parentType === 'post' ? 'post_reply' : 'story_reply',
                    metadata: {
                        comment_id,
                        reply_id: data[0].id,
        ...
        // Optionally: notify thread/post/story author if different from comment author and replier
...
const getReplies = async (
    ...
    // Fetch parent comment to determine thread/post/story context
    const { data: commentData, error: commentError } = await supabase
        .from('threadcomments')
...
const updateReplyReaction = async (
  ...
  // Determine if this is a thread, post, or story reply
  let parentType: 'thread' | 'post' | 'story', parentId: string, parentTitle: string, parentAuthorId: string;
  ...
  if (existing) {
    if (existing.type === type) {
      ...
      if (shouldNotify && authorProfile) {
        await sendNotification({
          ...
          type: parentType === 'thread' ? 'reaction_removed' : 
                parentType === 'post' ? 'post_reply_reaction_removed' : 'story_reply_reaction_removed',
          metadata: {
            reaction_type: type,
            reply_id,
    ...
    if (shouldNotify && authorProfile) {
      await sendNotification({
        ...
        type: parentType === 'thread' ? 'reaction_updated' : 
              parentType === 'post' ? 'post_reply_reaction_updated' : 'story_reply_reaction_updated',
        metadata: {
    ...
    if (shouldNotify && authorProfile) {
      // Define soulpoints for different reaction types
