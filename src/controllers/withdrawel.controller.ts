...
export const createwithdrawalsession= async (req: Request, res: Response) => {
  try {
    const { acAmount, grossAmountUSD, taxAmount, netAmountUSD, metadata } = req.body;
    ...
    const { data: pendingWithdrawal, error: insertError } = await supabase
      .from('withdrawal_transactions')
      .insert({
    ...
    const withdrawalMetadata: WithdrawalMetadata = {
      userId: metadata.userId,
      userEmail: metadata.userEmail || '',
      userName: metadata.userName || '',
      acAmount: acAmount.toString(),
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...withdrawalMetadata,
        internalTransactionId: transactionId, // Link to our internal transaction
      },
...
export const processwithdrawalsuccess=  async (req: Request, res: Response) => {
  try {
    const { sessionId } = req.body;
    ...
    const { data: currentAnamCoins, error: fetchAnamCoinsError } = await supabase
      .from('anamcoins')
      .select('*')
      .eq('user_id', userId)
      .single();
    ...
    if (!fetchAnamCoinsError && currentAnamCoins) {
      const { error: updateAnamCoinsError } = await supabase
        .from('anamcoins')
        .update({
...
export const useridstats= async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
export const transactiontransactionid=async (req: Request, res: Response) => {
  try {
